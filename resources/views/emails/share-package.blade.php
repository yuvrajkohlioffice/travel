...
<body class="bg-gray-100 p-6">
    <div class="max-w-2xl mx-auto bg-white rounded-xl shadow-md p-6">
        ...
        <p class="text-gray-700 mb-4">We are excited to share the details of the package <strong
                class="text-blue-700">{{ $packageName }}</strong> with you.</p>

        @if (count($documents) > 0)
            <p class="text-gray-700 mb-2 font-semibold">Attached Documents:</p>
            <ul class="list-disc list-inside mb-4">
                @foreach ($documents as $doc)
                    <li><a href="{{ $doc }}" target="_blank"
                            class="text-blue-600 underline">{{ $packageName }}</a></li>
                @endforeach
            </ul>
        @else
        ...
        <div class="mt-6">
            <a href="#"
                class="inline-block bg-blue-600 text-white px-5 py-3 rounded-lg hover:bg-blue-700 transition">Explore
                More Packages</a>
        </div>
